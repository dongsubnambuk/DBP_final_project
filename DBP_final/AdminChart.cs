            ...
            using (OracleConnection conn = new OracleConnection(connectionString))
            {
                ...
                using (OracleCommand cmd = new OracleCommand(query, conn))
                {
                    using (OracleDataReader reader = cmd.ExecuteReader())
                    {
                        ...
                        chart.ChartAreas[0].AxisX.LabelStyle.Angle = -45; // X축 레이블 기울기
                        chart.ChartAreas[0].AxisX.Interval = 1; // X축 간격
                        chart.Legends[0].Docking = Docking.Top; // 범례 위치
                    }
                }
            }
        }
                ...
                using (OracleCommand cmd = new OracleCommand(query, conn))
                {
                    using (OracleDataReader reader = cmd.ExecuteReader())
                    {
                        ...
                        while (reader.Read())
                        {
                            ...
                            series.Points.AddXY(professorName, courseCount);
                        }
                ...
                using (OracleCommand cmd = new OracleCommand(query, conn))
                {
                    using (OracleDataReader reader = cmd.ExecuteReader())
                    {
                        ...
                        while (reader.Read())
                        {
                            ...
                            series.Points.AddXY(courseName, studentCount);
                        }
                ...
                using (OracleCommand cmd = new OracleCommand(query, conn))
                {
                    using (OracleDataReader reader = cmd.ExecuteReader())
                    {
                        ...
                        while (reader.Read())
                        {
                            ...
                            series.Points.AddXY(studentName, avgGrade);
                        }
